...
class WechatCateController extends Controller
{
    ...
    {
        $data = DB::table('wechatcate')->where('status',2)->get();
...
//        dd($data);
        if($data['pid'] != null){
            //二级菜单添加
            $data['status'] = 3;
            $result = DB::table('wechatcate')->where('pid',$data['pid'])->count();
            if($result >= 5){
        ...
        //该分类下要添加二级分类  status应为2   2级菜单的意思
            $data['status'] = 2;
            $result = DB::table('wechatcate')->where('pid',0)->count();
            if($result >= 3){
        ...
        $res = DB::table('wechatcate')->insert($data);
        if($res){
...
//        dd($data);
    ...
    {
        $data = DB::table('wechatcate')->get()->toArray();//一级分类
...
//        dd($data);
        ...
        $dd = json_decode($res,true);
            if($dd['errmsg'] == 'ok'){
                ...
                echo "<script>alert('出现不可预知的错误');history.back(0)</script>";
            }
